...
public class PlayerInteractionHandler : MonoBehaviour
{
    ...
    {
        KeyInputs();
        pickDistance = Mathf.Clamp(pickDistance + Input.mouseScrollDelta.y * scrollSpeed, minGrabDistance, maxGrabDistance);
    ...
    private void LateUpdate()
    {
        if (selectedObject)
        {
            var midpoint = playerCamera.transform.position + pickDistance * .5f * playerCamera.transform.forward;
    ...
    private void FixedUpdate()
    {
        if (selectedObject != null)
        {
            ...
            selectedObjectPosition = ray.origin + ray.direction * pickDistance - selectedObject.transform.TransformVector(grabOffset);
    ...
    {
        selectedObject.collisionDetectionMode = CollisionDetectionMode.Discrete;
