...
public class PlayerGrapplingController : MonoBehaviour
{
    ...
    void AirMovement()
    {
        ...
        // right thrust
        if (Input.GetKey(KeyCode.D)) rb.AddForce(horizontalThrustForce * Time.deltaTime * playerOrientation.right);
        // left thrust
        if (Input.GetKey(KeyCode.A)) rb.AddForce(horizontalThrustForce * Time.deltaTime * -playerOrientation.right);

        // forward thrust
        if (Input.GetKey(KeyCode.W)) rb.AddForce(horizontalThrustForce * Time.deltaTime * playerOrientation.forward);
